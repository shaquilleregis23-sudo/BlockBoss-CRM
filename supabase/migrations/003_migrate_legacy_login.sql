-- Safely attach an authenticated Supabase user to their existing legacy team.
-- The Auth email must match the legacy account email and the old PIN must match.
...
returns table(team_id text, role text, display_name text)
language plpgsql
security definer
...
begin
  ...
  return query select v_team, v_role, coalesce(v_name,'');
end;
$$;

revoke all on function public.migrate_legacy_account(text,text,text) from public;
