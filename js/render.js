...
<div class="card internal-hide"><h3>⚙️ Settings</h3><div class="form-grid-2"><div class="form-row"><label>CRM / Company Name</label><input id="cfgCompany" value="${esc(b.company||'BlockBoss CRM')}"></div><div class="form-row"><label>Agent Name</label><input id="cfgAgent" value="${esc(b.agent_name||'Shaquille')}"></div></div><div class="form-grid-2"><div class="form-row"><label>Territory</label><input id="cfgTerritory" value="${esc(b.territory||'Queens / Long Island')}"></div><div class="form-row"><label>Plan</label><select id="cfgPlan"><option>Trial</option><option>Starter</option><option>Growth</option><option>Office</option></select></div></div><div class="form-grid-2"><div class="form-row"><label>Daily Door Goal</label><input type="number" id="cfgDoor" value="${b.door_goal||25}"></div><div class="form-row"><label>Daily Appointment Goal</label><input type="number" id="cfgAppt" value="${b.appt_goal||2}"></div></div><label class="row" style="margin-top:10px"><input type="checkbox" id="cfgLLC" ${b.include_llc!==false?'checked':''}> <span class="sub">Include LLC / Trust owners in PLUTO loads</span></label><label class="row" style="margin-top:8px"><input type="checkbox" id="cfgVerify" ${b.allow_verify!==false?'checked':''}> <span class="sub">Include borderline owner records</span></label><button class="save-btn purple" data-action="saveSettings">Save Settings</button></div>
<div class="card" style="border-color:rgba(63,185,80,.3)"><h3>🎁 Referral Program</h3><p class="sub">Share your link. Earn <strong style="color:var(--green)">$50 credit</strong> for every team that signs up through it.</p><div id="refBlock"><p class="sub">Loading your referral link…</p></div><button class="save-btn" onclick="(function(){var b=getBilling(),code=b.referral_code;if(!code){toast('Log in to see your referral link');return}var url='https://m2-energy-crm.netlify.app?ref='+code;var el=document.getElementById('refBlock');if(el)el.innerHTML='<div style=&quot;background:#0d1117;border:1px solid rgba(63,185,80,.3);border-radius:8px;padding:12px;margin:10px 0&quot;><div style=&quot;font-size:11px;color:var(--muted);margin-bottom:6px&quot;>YOUR REFERRAL LINK</div><div style=&quot;font-size:13px;color:#58a6ff;font-family:monospace;word-break:break-all&quot;>'+url+'</div></div><div class=&quot;metric-grid&quot; style=&quot;margin-top:10px&quot;><div class=&quot;metric&quot;><div class=&quot;k&quot;>Your Code</div><div class=&quot;v&quot; style=&quot;font-size:16px;font-family:monospace&quot;>'+code+'</div></div><div class=&quot;metric&quot;><div class=&quot;k&quot;>Credits Earned</div><div class=&quot;v&quot; style=&quot;color:var(--green)&quot;>$'+(b.referral_credits||0)*50+'</div></div></div>';navigator.clipboard.writeText(url).then(function(){toast('✓ Referral link copied!')}).catch(function(){toast('Link: '+url)})})()">Copy My Referral Link</button></div>
<div class="card" style="border-color:rgba(161,113,247,.3)"><h3>🎨 White-Label Branding</h3><p class="sub">Agency plan only — customize your team's CRM with your own logo and brand color.</p><div id="wlBlock"><p class="sub" style="font-size:12px">Available on Agency plan ($349/mo)</p></div><button class="save-btn purple" onclick="(function(){var b=getBilling(),plan=billingPlan();if(!plan||plan.agents<999){toast('White-label is an Agency plan feature — upgrade to unlock');return}var el=document.getElementById('wlBlock');if(el)el.innerHTML='<div class=&quot;form-row&quot;><label>Logo URL</label><input id=&quot;wlLogo&quot; placeholder=&quot;https://yourcompany.com/logo.png&quot; value=&quot;'+(b.logo_url||'')+' &quot;></div><div class=&quot;form-row&quot;><label>Accent Color</label><input id=&quot;wlColor&quot; type=&quot;color&quot; value=&quot;'+(b.accent_color||'#58a6ff')+'&quot;></div><button class=&quot;save-btn blue&quot; onclick=&quot;(function(){var logo=document.getElementById(\'wlLogo\').value.trim(),color=document.getElementById(\'wlColor\').value;saveBilling({logo_url:logo,accent_color:color});if(sb&&session().email){sb.from(\'master_accounts\').update({logo_url:logo,accent_color:color}).eq(\'email\',session().email).then(function(){toast(\'✓ Branding saved\');renderBrand()})}else{renderBrand();toast(\'✓ Preview applied — log in to save permanently\')}})()\">Save Branding</button>';})()">Customize Branding</button></div>
...
function renderList() {
  ...
  c.innerHTML = arr.slice(0, 700).map(l => `<div class="lead-item" data-id="${l.id}"><div class="nm">${esc(nameOf(l))} ${l.entity?'🏢':''}</div><div class="ad">${esc(l.addr||'')} ${l.boro?`· ${esc(l.boro)}`:''}</div><div class="row"><span class="badge ${statusBadge(l.status)}">${LABEL[l.status||'fresh']}</span><span class="badge gold">☀ ${sunScore(l)}</span><span class="badge blue">Q${leadQuality(l)}</span>${l.assigned_agent?`<span class="badge hot">👤 ${esc(l.assigned_agent)}</span>`:''}</div></div>`).join('');
